...
class HeaderLinkGroup(mb.BaseCKEditorModelActiveSortOrderHistorical):
    ...
    def get_admin_detail_v1_url(self):
        return reverse("admin-v1-setting-header-group-detail", kwargs={"pk": self.pk})

    def get_admin_update_v1_url(self):
        return reverse("admin-v1-setting-header-group-edit", kwargs={"pk": self.pk})

    def __str__(self):
        return self.title


class HeaderLink(mb.BaseCKEditorModelActiveSortOrderHistorical):
    group = models.ForeignKey(
        HeaderLinkGroup, on_delete=models.CASCADE, related_name="parent"
    )
    title = models.CharField(max_length=200)
    link = models.URLField(null=True, blank=True)
    open_in_newtab = models.BooleanField(default=False)
    ...
    def get_admin_detail_v1_url(self):
        return reverse("admin-v1-setting-header-link-detail", kwargs={"pk": self.pk})

    def get_admin_update_v1_url(self):
        return reverse("admin-v1-setting-header-link-edit", kwargs={"pk": self.pk})
